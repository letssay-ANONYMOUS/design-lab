import { c as tokensToVars, o as getFontPair, s as getPalette } from "./tokens-CLCekMxW.js";
...
*/
function resolveTone(tone, tones, fallback) {
...
*/
function passesTrustDensity(component, density) {
...
function visibleComponents(section, density) {
...
function scaleCount(density, min, max) {
...
*/
var H1 = "font-[family-name:var(--dl-font-heading)] text-[length:var(--dl-size-h1)] leading-[1.03] tracking-[var(--dl-tracking-h)] font-bold text-balance";
var H2 = "font-[family-name:var(--dl-font-heading)] text-[length:var(--dl-size-h2)] leading-[1.15] tracking-[var(--dl-tracking-h)] font-bold text-balance";
var H3 = "font-[family-name:var(--dl-font-heading)] text-[length:var(--dl-size-h3)] leading-[1.3] tracking-[var(--dl-tracking-h)] font-semibold";
var LEAD = "text-[length:var(--dl-size-lg)] leading-[1.55]";
var BODY = "text-[length:var(--dl-size-base)] leading-[1.6]";
var SMALL = "text-[length:var(--dl-size-sm)] leading-[1.5]";
var EYEBROW = `${SMALL} font-semibold uppercase tracking-[0.12em]`;
var SECTION = "py-[var(--dl-pad-y)] px-[var(--dl-pad-x)]";
var CONTAINER = "mx-auto w-full max-w-[1180px]";
var s = (value) => `{${JSON.stringify(value)}}`;
...
*/
function netTags(line) {
	if (line.startsWith("{/*") || line.startsWith("{\"")) return 0;
	let net = 0;
	for (const [tag] of line.matchAll(/<\/?[A-Za-z][^<>]*?\/?>/g)) if (tag.startsWith("</")) net -= 1;
...
*/
function reindent(jsx) {
	const out = [];
	let depth = 0;
	let inAttrs = false;
	for (const raw of jsx.split("\n")) {
		const line = raw.trim().replace(/className="([^"]*)"/g, (_, value) => `className="${value.trim().replace(/\s+/g, " ")}"`);
		if (!line) continue;
		if (inAttrs) {
			if (line === ">" || line === "/>") {
				...
				inAttrs = false;
				if (line === ">") depth += 1;
			} else out.push("  ".repeat(depth + 1) + line);
			continue;
		}
...
function componentName(section, index) {
	return `${section.type.charAt(0).toUpperCase() + section.type.slice(1)}${index + 1}`;
}
function copy(c, ctx, field = "text") {
...
var textColor = (ctx) => ctx.loud ? "text-[var(--dl-loud-text)]" : "text-[var(--dl-text)]";
var mutedColor = (ctx) => ctx.loud ? "text-[var(--dl-loud-text)]/80" : "text-[var(--dl-muted)]";
function emitBadge(c, ctx) {
	return `<span className="inline-flex w-fit items-center rounded-[var(--dl-radius-pill)] border px-3 py-1.5 ${SMALL} font-semibold uppercase tracking-[0.04em] ${ctx.loud ? "border-[var(--dl-loud-text)]/30 bg-[var(--dl-loud-text)]/10 text-[var(--dl-loud-text)]" : "border-[var(--dl-primary)]/28 bg-[var(--dl-primary)]/8 text-[var(--dl-primary)]"}">
  ${s(copy(c, ctx))}
</span>`;
}
function emitHeading(c, ctx, level = 1) {
	const cls = level === 1 ? H1 : level === 2 ? H2 : H3;
	const tag = `h${level}`;
	return `<${tag} className="${cls} ${textColor(ctx)}">${s(copy(c, ctx))}</${tag}>`;
}
function emitParagraph(c, ctx, size = LEAD) {
	return `<p className="${size} ${mutedColor(ctx)} max-w-[62ch]">${s(copy(c, ctx))}</p>`;
}
function emitButton(c, ctx) {
	const emphasis = c.props.emphasis ?? "primary";
	return `<button type="button" className="inline-flex items-center justify-center rounded-[var(--dl-radius)] px-6 py-3.5 ${BODY} font-semibold transition-transform duration-150 hover:-translate-y-px ${emphasis === "primary" ? ctx.loud ? "bg-[var(--dl-loud-text)] text-[var(--dl-loud-bg)]" : "bg-[var(--dl-primary)] text-[var(--dl-primary-fg)] shadow-[0_8px_22px_-12px_rgba(0,0,0,.45)]" : emphasis === "secondary" ? "bg-[var(--dl-surface)] text-[var(--dl-text)] border border-[var(--dl-border)]" : ctx.loud ? "border border-[var(--dl-loud-text)]/30 text-[var(--dl-loud-text)]" : "border border-[var(--dl-primary)]/25 text-[var(--dl-primary)]"}">
  ${s(copy(c, ctx))}
</button>`;
}
function emitImage(c, ratio) {
	const fills = [
...
function emitStars(c, ctx) {
	const filled = Math.round(c.props.rating ?? 5);
	const stars = Array.from({ length: 5 }, (_, i) => i < filled ? `<svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor" aria-hidden><path d="${STAR_PATH}" /></svg>` : `<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" opacity="0.35" aria-hidden><path d="${STAR_PATH}" /></svg>`).join("\n");
...
function emitQuote(c, ctx) {
	return `<figure className="m-0 flex flex-col gap-[var(--dl-gap-sm)] rounded-[var(--dl-radius)] border p-[var(--dl-gap)] ${ctx.loud ? "bg-[var(--dl-loud-text)]/8 border-[var(--dl-loud-text)]/16" : "bg-[var(--dl-surface)] border-[var(--dl-border)]"}">
...
function emitFeature(c, ctx) {
	return `<div className="flex flex-col gap-[var(--dl-gap-sm)]">
  <span className="flex h-11 w-11 items-center justify-center rounded-[var(--dl-radius-sm)] ${ctx.loud ? "bg-[var(--dl-loud-text)]/14 text-[var(--dl-loud-text)]" : "bg-[var(--dl-primary)]/10 text-[var(--dl-primary)]"}">
...
function emitPrice(c, ctx) {
	const featured = c.props.featured;
	const skin = featured ? "bg-[var(--dl-loud-bg)] text-[var(--dl-loud-text)] shadow-[0_24px_60px_-30px_rgba(0,0,0,.55)]" : "bg-[var(--dl-surface)] border border-[var(--dl-border)]";
	const bullets = (c.props.bullets ?? []).map((bullet) => `<li className="flex items-start gap-2.5 ${BODY} ${featured ? "text-[var(--dl-loud-text)]/85" : mutedColor(ctx)}">
...
function emitLogos(c, ctx, count) {
	return `<div className="flex flex-wrap items-center justify-center gap-x-[var(--dl-gap-lg)] gap-y-[var(--dl-gap)]">
${indent((c.props.logos ?? []).slice(0, Math.max(2, count)).map((logo) => `<span className="font-[family-name:var(--dl-font-heading)] ${BODY} font-semibold opacity-55 ${textColor(ctx)}">${s(logo)}</span>`).join("\n"), 1)}
</div>`;
}
function group(section, view) {
	const visible = section.components.filter((c) => passesTrustDensity(c, view.trustDensity));
	const of = (type) => visible.filter((c) => c.type === type);
	return {
		badges: of("badge"),
		headings: of("heading"),
...
function join(blocks, depth = 1) {
	return blocks.filter(Boolean).map((block) => indent(block, depth)).join("\n");
}
function emitHeader(g, ctx, align) {
	const parts = [
		...g.subheadings.map((c) => `<span className="${EYEBROW} ${ctx.loud ? "text-[var(--dl-loud-text)]/80" : "text-[var(--dl-primary)]"}">${s(copy(c, ctx))}</span>`),
		...g.headings.map((c) => emitHeading(c, ctx, 2)),
		...g.paragraphs.map((c) => emitParagraph(c, ctx))
	];
	if (parts.length === 0) return "";
	return `<div className="mb-[var(--dl-gap-lg)] flex flex-col gap-[var(--dl-gap-sm)] ${align === "center" ? "items-center text-center mx-auto max-w-[760px]" : "items-start max-w-[720px]"}">
${join(parts)}
</div>`;
}
function emitSection(section, view) {
	const ctx = {
		view,
		loud: section.mood === "loud"
	};
	const g = group(section, view);
	const bg = ctx.loud ? "bg-[var(--dl-loud-bg)] text-[var(--dl-loud-text)]" : "bg-[var(--dl-bg)] text-[var(--dl-text)]";
	const copyStack = (align) => join([
		...g.badges.map((c) => emitBadge(c, ctx)),
		...g.headings.map((c) => emitHeading(c, ctx, 1)),
		...g.paragraphs.map((c) => emitParagraph(c, ctx)),
		g.buttons.length ? `<div className="flex flex-wrap gap-[var(--dl-gap-sm)] ${align === "center" ? "justify-center" : ""}">
${join(g.buttons.map((c) => emitButton(c, ctx)))}
</div>` : "",
		...g.stars.map((c) => emitStars(c, ctx)),
		g.stats.length ? `<div className="flex flex-wrap gap-[var(--dl-gap-lg)]">
...
${image ? indent(`<div className="absolute inset-0 -z-20 scale-110">\n${indent(emitImage(image, "auto"), 1)}\n</div>`, 1) : ""}
  <div className="absolute inset-0 -z-10" style={{ background: 'linear-gradient(to top, rgba(0,0,0,${(alpha + .18).toFixed(2)}), rgba(0,0,0,${alpha.toFixed(2)}))' }} />
...
${copyStack("left")}
			...
			if (section.variant === "centered" || section.variant === "minimal") return `<section className="${SECTION} ${bg}">
...
${join(g.features.map((c) => `<div className="h-full rounded-[var(--dl-radius)] border p-[var(--dl-gap)] ${ctx.loud ? "border-[var(--dl-loud-text)]/14 bg-[var(--dl-loud-text)]/7" : "border-[var(--dl-border)] bg-[var(--dl-surface)]"}">
...
${g.stats.length ? indent(`<div className="mt-[var(--dl-gap-lg)] grid gap-[var(--dl-gap)] border-t border-[var(--dl-border)] pt-[var(--dl-gap-lg)] md:grid-cols-${g.stats.length}">
${join(g.stats.map((c) => emitStat(c, ctx)), 1)}
</div>`, 2) : ""}
  </div>
</section>`;
		}
		case "pricing": return `<section className="${SECTION} ${bg}">
  <div className="${CONTAINER}">
...
${join(g.prices.map((c) => emitPrice(c, ctx)), 3)}
		...
		case "faq": return `<section className="${SECTION} ${bg}">
...
${join(g.faqs.map((c) => emitFaq(c, ctx)), 3)}
		...
		case "bento": return `<section className="${SECTION} ${bg}">
  <div className="${CONTAINER}">
${indent(emitHeader(g, ctx, "left"), 2)}
    <div className="grid auto-rows-[minmax(148px,auto)] grid-cols-12 gap-[var(--dl-gap-sm)]">
${join(g.cards.map((c) => {
			const span = c.props.span ?? {
				col: 4,
				row: 1
			};
			return `<div className="flex flex-col overflow-hidden rounded-[var(--dl-radius)] border ${ctx.loud ? "border-[var(--dl-loud-text)]/16 bg-[var(--dl-loud-text)]/8" : "border-[var(--dl-border)] bg-[var(--dl-surface)]"}" style={{ gridColumn: 'span ${span.col}', gridRow: 'span ${span.row}' }}>
    ...
    <p className="${BODY} ${mutedColor(ctx)}">${s(copy(c, ctx, "sub"))}</p>
		...
		case "ctaBanner": {
			...
			const inner = join([
				...
				...g.headings.map((c) => emitHeading(c, ctx, 2)),
				...g.paragraphs.map((c) => emitParagraph(c, ctx)),
				g.buttons.length ? `<div className="mt-[var(--dl-gap-sm)] flex flex-wrap justify-center gap-[var(--dl-gap-sm)]">
${join(g.buttons.map((c) => emitButton(c, ctx)))}
</div>` : ""
			], 3);
			if (panel) return `<section className="${SECTION} bg-[var(--dl-bg)]">
  <div className="${CONTAINER} flex flex-col items-center gap-[var(--dl-gap-sm)] rounded-[var(--dl-radius-lg)] px-[var(--dl-pad-x)] py-[calc(var(--dl-pad-y)*0.7)] text-center ${ctx.loud ? "bg-[var(--dl-loud-bg)] text-[var(--dl-loud-text)]" : "border border-[var(--dl-border)] bg-[var(--dl-surface)]"}">
${inner}
  </div>
</section>`;
			return `<section className="${SECTION} ${bg}">
...
${inner}
		...
		case "footer": return `<footer className="${SECTION} ${ctx.loud ? "bg-[var(--dl-loud-bg)] text-[var(--dl-loud-text)]" : "bg-[var(--dl-surface)] text-[var(--dl-text)]"}">
...
${join(g.listItems.map((c) => `<span className="${BODY} ${mutedColor(ctx)}">${s(copy(c, ctx))}</span>`), 3)}
    </nav>
  </div>
</footer>`;
	}
}
function generatePageJsx(page, view) {
	...
	const varLines = Object.entries(vars).map(([key, value]) => `  ${JSON.stringify(key)}: ${JSON.stringify(value)},`).join("\n");
	const bodies = page.sections.map((section, i) => {
...
${indent(jsx, 2)}
	...
	const calls = page.sections.map((section, i) => `      <${componentName(section, i)} />`).join("\n");
	return `/**
 * ${page.name}
 * Generated by Design Lab — ${(/* @__PURE__ */ new Date()).toISOString().slice(0, 10)}
 *
...
//#endregion
export { visibleComponents as i, resolveTone as n, scaleCount as r, generatePageJsx as t };
